...
class game {
  ...
  crash (arr, type) {
    let result = arr.filter(value => value !== ' ') // 把空格去掉
    let temp = arr.filter(value => value !== ' ') // 把空格去掉
    temp.forEach((value, index, arr) => {
      if (value === arr[index + 1] && value !== ' ' && index < this.chessNumber) {
        result.splice(index, 2, value * 2)
      }
    })
    if (type === 'left') {
      while (result.length < this.chessNumber) {
        result.push(' ')
      }
    }
    return result
